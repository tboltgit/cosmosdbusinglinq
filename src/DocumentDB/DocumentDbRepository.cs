...
namespace DocumentDB
{
    public class DocumentDbRepository<T> : IDatabaseRepository<T>, IDisposable where T : class
    {
        ...
        public async Task<IQueryable<T>> GetAllAsync(string sqlQuery, string token, int pageSize, string partitionKey = null)
        {
        ...
        public async Task<PaginatedResponse<T>> GetManyWithPaginationAsync(string token, int pageSize,
            Expression<Func<T, bool>> predicate, string sortColumn, string sortOrder, string partitionKey = null)
        {
            var link = UriFactory.CreateDocumentCollectionUri(_databaseId, _collectionName);
            ...
            if (!String.IsNullOrWhiteSpace(sortColumn))
                querySetup = OrderByColumn(querySetup, sortColumn, sortOrder);
            ...
            return new PaginatedResponse<T>(result.AsQueryable(), result.ResponseContinuation);
        }
        ...
        private IQueryable<T> OrderByColumn<T>(IQueryable<T> queryable, string sortColumn, string sortOrder = "ASC")
        {
            ...
            Expression propertyExpression = sortColumn.Split('.')
                                            .Aggregate<string, Expression>
                                            (parameterExpression, (c, m) => Expression.Property(c, m));

            var selector = Expression.Lambda(propertyExpression, parameterExpression);

            var orderByExpression = Expression.Call(typeof(Queryable), orderByMethodName,
                new[] { elementType, propertyExpression.Type }, queryable.Expression, selector);
